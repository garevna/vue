...
// ====================================================== Components
sectionPosts ['Components'] = [
  {
    head: "Radio button",
    pict: false,
    text: `Простой компонент с литеральным шаблоном. 
            Использование директив v-for, v-bind, v-model 
            при вызове компонента. Передача данных из 
            компонента родителю с помощью встроенного метода 
            $emit`,
    ref:[
  ...
  },
  {
    head: "dropdown menu",
    pict: false,
    text: `Пример компонента dropdown-menu, в котором при выборе 
            одного из элементов меню происходит отправка события 
            родителю (экземпляру Vue) с передачей внутреннего 
            содержимого выбранного пункта меню. В экземпляре 
            Vue происходит обработка события и вывод 
            соотвествующего текста на страницу`,
    ref:[
...
sectionPosts ['Templates'] = [
  ...
  },
  {
    head: "Тег <template>",
    pict: false,
    text: `Шаблон можно описать в теге <template> с атрибутом id,
            значение которого потом использовать для подключения 
            шаблона к компоненту`,
    code:`
  ...
  },
  {
    ...
    text: `При условии использованя инструментов сборки 
          ...
          с раширением vue. 
          Если тег <style> имеет атрибут scoped, его CSS будет 
          применяться только к элементам текущего компонента. 
          Это похоже на инкапсуляцию стиля в Shadow DOM. 
